...
def _source_name(path):
    """从 sources/<name>.json 路径提取 Skill 名称。"""
    parts = Path(path).parts
    if len(parts) == 2 and parts[0] == "sources" and parts[1].endswith(".json"):
        return Path(parts[1]).stem
    return None


def _changed_source_names(changed_paths):
    """返回本次变化的独立配置名称。"""
    return {name for path in changed_paths if (name := _source_name(path))}


def _controlled_skill_names(branch, changed_paths):
    """校验自动化 PR 路径，并返回实际受影响的 Skill。"""
    source_names = _changed_source_names(changed_paths)
    skill_names = {
        parts[1]
    ...
    if branch == SYNC_BRANCH:
        allowed_names = source_names | skill_names
    elif COLLECTION_BRANCH_PATTERN.fullmatch(branch):
        if len(source_names) != 1 or skill_names != source_names:
            raise ValueError("收录 PR 必须且只能新增一个配置及其 Skill 目录")
    ...
    invalid_paths = sorted(
        path
        for path in changed_paths
        if _source_name(path) not in allowed_names
        and not any(path.startswith(f"skills/{name}/") for name in allowed_names)
    )
    if invalid_paths:
        raise ValueError(f"PR 包含越界文件：{', '.join(invalid_paths)}")
    if not allowed_names:
        raise ValueError("PR 没有 Skill 变更")
...
def _validate_merge_candidate(repository, pr_number, pull):
    ...
    affected_names = _controlled_skill_names(pull["head"]["ref"], changed_paths)
    ...
    if COLLECTION_BRANCH_PATTERN.fullmatch(pull["head"]["ref"]):
        name = next(iter(affected_names))
        if _run(
            ["git", "cat-file", "-e", f"{base_sha}:sources/{name}.json"],
            capture=True,
            check=False,
        ).returncode == 0:
            raise ValueError(f"最新默认分支已包含 {name}")
    with tempfile.TemporaryDirectory(prefix="codex-pr-") as directory:
        ...
        try:
            errors = validate_catalog(checkout, check_readme=False)
...
def _load_sync_module(root):
    ...
    module.ROOT = root
    module.SOURCES_PATH = root / "sources"
...
def _valid_category(category):
    ...
    return (
        ...
        and re.fullmatch(r"[\u4e00-\u9fff]{2,12}", category) is not None
        and not any(word in category for word in GENERIC_CATEGORIES)
    )


def validate_catalog(root, check_readme=True):
    """验证目录、独立配置、描述和静态安全约束。"""
    errors = []
    ...
    for source in sources:
        ...
        if not _valid_category(source["category"]):
            errors.append(f"{source['name']} 的分类应为 2–12 个中文字符")
    ...
    for name in sorted(actual_names - expected_names):
        errors.append(f"未在 sources 登记的目录：skills/{name}")
    ...
    if check_readme:
        try:
            ...
            if sync._render_readme(readme, sources) != readme:
                errors.append("README 技能清单与 sources 不一致，请运行重建流程")
...
def verify_upstreams(root, skill_names):
    ...
    with tempfile.TemporaryDirectory(prefix="codex-upstream-") as directory:
        ...
        for index, name in enumerate(sorted(skill_names)):
            ...
            if source is None:
                errors.append(f"{name} 已从 sources 删除，收录管家不处理删除")
                continue
...
def _issue_skill_path(body):
    """读取收录模板中的 Skill 路径，缺失时不猜测。"""
    match = SKILL_PATH_PATTERN.search(body or "")
    if not match:
        return None
    return match.group(1).strip().splitlines()[0].strip().strip("`") or "."


def _relative_skill_path(value):
    """仅接受候选仓库内的相对 Skill 路径。"""
    if "\0" in value:
        return None
    path = Path(value)
...
def _changed_skill_names(pull_files):
    """从 PR 文件列表中提取实际变化的 Skill 目录名。"""
    names = set()
...
def _preflight_result(reason, pr_number=None, expected_head_sha=None):
    """把确定性收录阻断项转换为可直接执行的结构化结论。"""
    return {
        "action": "comment",
...
def prepare_context(
    ...
    """生成当前 Issue 或 PR 的最小只读审查上下文。

    Args:
        event_path: GitHub 事件 JSON 路径。
        output_directory: 上下文与候选上游的输出目录。
        expected_head_sha: 必须与当前 PR Head 一致的可选提交 SHA。
        include_comments: 是否包含当前 Issue 或 PR 的评论。
        github_output: 确定性前置检查失败时写入结果的 GitHub 输出文件。

    Returns:
        None。结果写入 ``output_directory/context.json``。
    """
    ...
    candidate_urls = _repository_urls([instruction], repository) or _repository_urls(
        [issue.get("body")], repository
    )
    candidate_path = _issue_skill_path(issue.get("body"))
    preflight_failure = None
    if pull_request is not None or not include_comments:
        candidate_urls = []
    elif len(candidate_urls) != 1:
        preflight_failure = "Issue 必须提供且只能提供一个 GitHub 仓库根地址"
    elif candidate_path is None:
        preflight_failure = "Issue 未提供明确的 Skill 路径"
    elif _relative_skill_path(candidate_path) is None:
        preflight_failure = "Skill 路径必须是候选仓库内的安全相对路径"
    else:
        sources = _load_sync_module(ROOT)._load_sources()
        normalized_path = str(Path(candidate_path))
        if any(
            source["repository"].lower() == candidate_urls[0].lower()
            and source["path"] == normalized_path
            for source in sources
        ):
            preflight_failure = "该仓库与 Skill 路径已经收录"

    for index, repository_url in enumerate(candidate_urls[:1]):
        target = upstream_root / str(index)
        record = {"repository": repository_url, "path": None, "clone_error": None}
        if preflight_failure is not None:
            upstreams.append(record)
            continue
        relative_candidate = _relative_skill_path(candidate_path)
        try:
            _run(
                [
                    "git",
            ...
            skill_root = target / relative_candidate
            if not _inside(skill_root, target):
                record["clone_error"] = "Skill 路径指向候选仓库之外"
                preflight_failure = record["clone_error"]
            elif skill_root.is_symlink() or not (skill_root / "SKILL.md").is_file():
                record["clone_error"] = "候选路径缺少真实的 SKILL.md"
                preflight_failure = record["clone_error"]
            else:
                record["path"] = str(skill_root)
                for finding in scan_skill_tree(skill_root):
                    static_findings.append({"scope": candidate_path, **finding})
                blocking = next(
                    (item for item in static_findings if item["level"] == "error"),
                    None,
                )
                if blocking:
                    preflight_failure = (
                        f"`{blocking['scope']}/{blocking['path']}`："
                        f"{blocking['reason']}"
                    )
        except (subprocess.CalledProcessError, subprocess.TimeoutExpired):
            record["clone_error"] = "候选上游仓库无法克隆"
            preflight_failure = "候选上游仓库无法克隆"
        upstreams.append(record)

    if pull_checkout is not None:
        for name in _changed_skill_names(pull_files):
            for finding in scan_skill_tree(pull_checkout / "skills" / name):
                static_findings.append({"scope": f"skills/{name}", **finding})
        blocking = next(
            (item for item in static_findings if item["level"] == "error"), None
        )
        if blocking:
            preflight_failure = (
                f"`{blocking['scope']}/{blocking['path']}`：{blocking['reason']}"
            )
    ...
    if preflight_failure is not None and github_output is not None:
        result = json.dumps(
            _preflight_result(
                preflight_failure,
                pull_request["number"] if pull_request else None,
                pull_request["checkout_sha"] if pull_request else None,
            ),
            ensure_ascii=False,
            separators=(",", ":"),
        )
...
def prepare_review_context(
    repository, pr_number, output_directory, expected_head_sha, github_output=None
):
    """为指定 Skill PR 生成不含仓库历史的最小审查上下文。

    Args:
        repository: ``owner/repository`` 格式的目标仓库。
        pr_number: 待审 PR 编号。
        output_directory: 上下文与只读 PR worktree 的输出目录。
        expected_head_sha: 准备上下文时必须保持不变的 PR Head SHA。
        github_output: 确定性前置检查失败时写入结果的 GitHub 输出文件。
...
def _create_pr(event, result):
    ...
    sync = _load_sync_module(ROOT)
    existing_sources = sync._load_sources()
    if any(source["name"] == candidate["name"] for source in existing_sources):
        raise ValueError(f"sources 已包含 {candidate['name']}")
    if any(
        source["repository"].lower() == candidate["repository"].lower()
        and source["path"] == str(Path(candidate["path"]))
        for source in existing_sources
    ):
        raise ValueError("该仓库与 Skill 路径已经收录")
    source = {**candidate, "updated_at": sync._current_updated_at()}
    sync._write_source(source)
    sync._load_sources()
    ...
    errors = validate_catalog(ROOT, check_readme=False)
    if errors:
        raise ValueError("；".join(errors))
...
def _dispatch_readme_rebuild(repository, default_branch):
    """显式触发只维护 README 的受控工作流。"""
    _run(
        [
            "gh", "workflow", "run", "rebuild-readme.yml",
            "--repo", repository,
            "--ref", default_branch,
        ]
    )
...
def _merge_pr(event, result):
    ...
    rebuild_readme = bool(
        _changed_source_names(
            {item["filename"] for item in _pull_files(repository, pr_number)}
        )
    )
    ...
    if rebuild_readme:
        try:
            _dispatch_readme_rebuild(
                repository, event["repository"]["default_branch"]
            )
        except Exception as error:
            cleanup_errors.append(f"启动 README 重建失败：{error}")
    if cleanup:
...
def _validate_automated_review(
    ...
    _controlled_skill_names(branch, changed_paths)
...
def self_check():
    ...
    assert _issue_skill_path(
        "### Skill 路径\n\nskills/demo\n\n### 用途说明\n\n示例"
    ) == "skills/demo"
    assert _issue_skill_path("没有结构化路径") is None
    assert _relative_skill_path("skills/demo") == Path("skills/demo")
    assert _relative_skill_path("../outside") is None
    assert _relative_skill_path("/outside") is None
    assert _relative_skill_path("bad\0path") is None
    assert _changed_skill_names([
        {"filename": "README.md"},
        {"filename": "skills/demo/SKILL.md"},
        {"filename": "skills/demo/references/prompt.md"},
    ]) == ["demo"]
    ...
    assert _controlled_skill_names("codex/issue-3-123", collection_paths) == {
        "new-skill"
    }
    other_collection_paths = {
        "sources/other-skill.json",
        "skills/other-skill/SKILL.md",
    }
    assert collection_paths.isdisjoint(other_collection_paths)
    try:
        _controlled_skill_names(
            "codex/issue-3-123",
            {"sources/new-skill.json", "skills/existing/SKILL.md"},
        )
    except ValueError:
        pass
    else:
        raise AssertionError("收录 PR 不得带入其他 Skill 变更")
    assert _preflight_result("缺少 SKILL.md")["action"] == "comment"
    assert _valid_category("插画与视觉创作")
    assert not _valid_category("其他")
    assert not _valid_category("AI 绘图")
    ...
    _validate_automated_review(
        ...
        7,
        sync_result,
        {"sources/example.json", "skills/example/SKILL.md"},
    )
...
def main():
    ...
    prepare_review.add_argument("--github-output", type=Path)
